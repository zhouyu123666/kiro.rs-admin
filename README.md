...
| `POST`   | `/v1/responses`        | Responses API（Codex 接入的唯一端点；流式 SSE、工具调用、WebSearch、`previous_response_id` 多轮、Remote Compaction v2） |
...
Codex 手动执行 `/compact` 或达到上下文阈值时，服务会识别 Responses input 末尾唯一的 `compaction_trigger`，使用 Kiro 生成续接摘要，并按 Remote Compaction v2 协议返回唯一的 `compaction` output item；该摘要会在后续请求中恢复为先前对话上下文。
...
| ------------------------------------- | -------------------------------- |
| `/admin`                              | 嵌入式 Web 管理界面              |
| `/api/admin/accounts/usage`           | 查询所有 Kiro 账号的最近用量快照，包含账号分组（`groups`）、来源渠道（`sourceChannel`）、RPM 上限（`rpmLimit`，0 表示不限速）和当前请求数（`rpmCurrent`） |
